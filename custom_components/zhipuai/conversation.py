...
class ZhipuAIConversationEntity(conversation.ConversationEntity, conversation.AbstractConversationAgent):
    ...
    async def async_process(self, user_input: conversation.ConversationInput) -> conversation.ConversationResult:
        ...
        if options.get(CONF_LLM_HASS_API):
            try:
                ...
                intent_response.async_set_error(intent.IntentResponseErrorCode.UNKNOWN, f"准备 LLM API 时出错： {err}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                for _iteration in range(MAX_TOOL_ITERATIONS):
                    ...
                    for tool_call in tool_calls:
                        ...
                        LOGGER.debug("工具调用: %s(%s)", tool_input.tool_name, tool_input.tool_args)
            ...
            try:
                ...
                LOGGER.error("获取助手 LLM 时出错: %s", e)
                intent_response.async_set_error(intent.IntentResponseErrorCode.UNKNOWN, f"获取助手 LLM 时出错: {e}")
                return conversation.ConversationResult(response=intent_response, conversation_id=conversation_id)
    ...
    async def _async_entry_update_listener(self, entry: ConfigEntry) -> None:
        self.entry = entry
...
async def async_setup_entry(
    ...
    async_add_entities([ZhipuAIConversationEntity(config_entry)])
