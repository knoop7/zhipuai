...
class ZhipuAIConversationEntity(conversation.ConversationEntity, conversation.AbstractConversationAgent):
                    ...
                    for tool_call in tool_calls:
                        try:
                            ...
                            if "entity_id" in tool_args:
                                entity_ids = tool_args["entity_id"]
                                ...
                                for entity_id in entity_ids:
                                    ...
                                    if state:
                                        if state.domain == "sensor":
                                            ...
                                            processed_entities[entity_id] = "non_sensor"
                                    else:
                                        processed_entities[entity_id] = "not_found"
                                
                                tool_args["processed_entities"] = processed_entities
                                if "entity_id" in tool_args:
                                    del tool_args["entity_id"]
                            ...
                            formatted_response = self._format_tool_response(tool_response, tool_name)
                            ...
                            LOGGER.error(f"工具调用参数错误: {ve}")
                            formatted_response = f"工具 {tool_name} 调用失败: 参数错误"
                        except Exception as e:
                            LOGGER.error(f"工具调用失败: {e}")
                            formatted_response = f"工具 {tool_name} 调用失败: 未知错误"
    ...
    def _format_tool_response(self, response: Any, tool_name: str) -> str:
        if isinstance(response, dict) and "processed_entities" in response:
            formatted = f"工具 {tool_name} 的执行结果:\n"
            for entity_id, info in response["processed_entities"].items():
                if isinstance(info, dict):
                    formatted += f"- {info['friendly_name']} ({entity_id}): {info['state']}"
                    if info['unit_of_measurement']:
                        formatted += f" {info['unit_of_measurement']}"
                    formatted += "\n"
                elif info == "non_sensor":
                    formatted += f"- {entity_id}: 非传感器实体，操作已执行\n"
                else:
                    formatted += f"- {entity_id}: 未找到该实体\n"
            return formatted
        else:
            return f"工具 {tool_name} 的执行结果: {json.dumps(response, ensure_ascii=False, indent=2)}"
